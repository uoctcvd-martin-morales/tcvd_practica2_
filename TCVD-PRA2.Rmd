...
Así a partir del resultado del test podemos concluir que no se da una homogeneidad en la varianza entre los distintos tipos de inmuebles.
...
Vemos que las variables Area y Frontage están correlacionadas, pero por si solas no explican el precio por metro cuadrado. Tendremos que ver la influencia de las otras variables a lo largo del tiempo.
...
tb0
```
Vemos que en 2014 hubo una bajada general, perodesde ese momento el valor del metro cuadrado ha ido creciendo. El punto aquí es que seguramente un el precio del metro de un piso no haya evolucionado igual que el una casa en mitad del bosque. Veamos como a los largo de los años se comporta el percio del metro cuadrado en diferente variables
...
tb2
```
Aquí vemos la misma situación que antes. No es lo mismo una vivienda en una zona comercial donde el precio es alto y se mantiene que en unra zona industrial que cada vez se va devaluando que en una "Quasi-residential Zone" donde en estos ultimos años empieza a valer bastante mas el metro cuadrado.
...
En un principio se pensaba hacer un modelo para estimar el valor del metro cuadrado a traves de **comparables**. El problema aqui es que por falta de experiencia del sector inmobiliario no pudimos encontrar una forma de encontrar un "inmueble parecido". El hecho de cómo ha evolucionado el precio del metro cuadrado en los últimos años y en la hetereogeneidad de este en diferentes prefectura y dentro de estas, en los diferentes municipios y  distritos nos impedía encontrar un inmueble similar a otro. Una opción con más tiempo sería aplicar un algoritmo de Clustering o un Árbol de decisión para esto. Como se pedía también una solución sencilla, hemos optado en seleccionar las variables que hemos visto que influyen en el precio del metro cuadrado y crear un modelo de regresión lineal múltiple con regresores cuantitativos y cualitativos.

Como hemos observado que la situación hace 5 años era muy diferente a la actual, únicamente me quedaré conlos datos de los 2 últimos años.
...
Creamos nuestro primer modelo, empleando únicamente las variables Area y Frontage que habíamos visto que presentan una fuerte correlación
...
Como se puede ver, el valor de R^2 es bastante bajo (0'2245) por lo que planteamos añadir algunas de las variables
categóricas del dataset (Type, Region, Prefecture, CityPlanning y Renovation) para observar si mejoran el modelo a crear:
...
```
Tras añadir estas variables el modelo mejora bastante (hasta un R^2 = 0'5628). 


Añadimos también MunicipalityCode y Structure a la regresión con el siguiente resultado:

```{r, warning=FALSE}
model3 <- lm(UnitPrice~factor(Type)+Area + Frontage+factor(Region)+ MunicipalityCode + factor(Prefecture)+
factor(CityPlanning)+factor(Renovation)+ factor(Structure), data=df_japan_train, na.action = na.exclude)
summary(model3)

```
Esto mejora ligeramente la regresión, por lo que lo mantenemos como modelo fianl.

Por último llevamos a cabo la predicción de los valores de TradePrice para los datos de test, obteniendo un error medio de -338353.1

```{r, warning=FALSE}

df_japan_test$NewValue <- predict(model3, df_japan_test, interval = c("confidence"))
...
mean(df_japan_result$TradePrice - df_japan_result$EstimateTradePrice)

```

# Resolución del problema

Como conclusión tras el estudio del dataset podemos concluir que la estimación del valor exacto de un inmueble atiende a un gran número
de variables, complicando en gran medida la generación de modelos predictivos. Aun así, el modelo generado, si bien sigue teniendo un margen 
de mejora bastante amplio, es capaz de reducir el error cometido por la tasación de inmuebles únicamente por su tamaño (primer modelo) y pone
de manifiesto la importancia de la ubicación y tipo de construcción de las viviendas a estudiar. 
